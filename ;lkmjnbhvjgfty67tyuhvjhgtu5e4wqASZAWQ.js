    ...
    var xhr1 = $.ajax({
        ...
        success: function(data) {
            ...
            $.each(lightsArray, function(i, lightInfo) {
                ...
                $("#dropdownBulb").append('<option value="' + lightInfo[0] + '">' + lightInfo[1] + " - " + state + '</option>');
            });
...
function updateLights() {
    var xhr1 = $.ajax({
        ...
        success: function(data) {
            $("#dropdownBulb").empty();
            $("#dropdownBulb").append('<option value="all">All Lights</option>');
            //this block of code parses the available lights and populates the dropdown to select them.
            var lightsArray = [];
            $.each(data, function(i, lightObject) {
                var lightTuple = []; // Here I'm using an internal temporary--
...
// Toggles Dark Theme / Light Theme Standard
$('#dark-mode').click(function() {
    // chrome.storage.local.get("darkTheme", function(result) {
    //     var themeState = result.darkTheme;
    //     if (themeState == true) {
    //         // Turn off dark theme
    //     } else if (themeState == undefined || themeState == false) {
            $('.mdl-card').css('background-color', 'rgb(0, 0, 0)');
            $('.logo').css('filter', 'invert(100%)');
            $('#settings img').css('filter', 'invert(100%)');
            $('#settings').css('background-color', 'rgb(0, 0, 0)');
            $('h2').css('color', 'white');
            $('h3').css('color', 'white');
            $('p').css('color', 'white');
            $('.mdl-card--border').css('border-top', 'none');
            $('.black').removeClass('black').addClass('white');
            var obj = { 'darkTheme': true };
            // Save it using the Chrome extension storage API.
            chrome.storage.local.set(obj, function() {
                // Notify that we saved.
        //     });
